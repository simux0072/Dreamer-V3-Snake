...
class Dreamer_V3(nn.Module):
    def __init__(self, input_dim, output_dim, sym_net, length) -> None:
        ...
        self.input_dim: list[int, int] = input_dim
        ...
        self.memory_module = nn.LSTM(input_size=3249, hidden_size=2048, num_layers=5, batch_first=True)
    ...
    def decoder_pass(self, disc_dist: torch.Tensor):
        return self.decoder(disc_dist.flatten(start_dim=-2)).reshape((1, 1, self.input_dim[0], self.input_dim[1]))
    
    @njit
    def create_dist_from_probs(self, input: torch.Tensor):
        disc_dist_probs: torch.Tensor = torch.softmax(torch.ones(input.size()) * 0.01 + input * 0.99)
    ...
    def forward_train_model(self, observations: torch.Tensor, actions: torch.Tensor):
        ...
        for index in range(observations.size(1)):
            encoded: torch.Tensor = self.encoder_pass(observations[0, index].unsqueeze(dim=0)) # Shape: (1, 1, X)
            disc_dist_base, disc_dist: torch.Tensor = self.dist_from_encoder(encoded) # Shape: (1, 1, X)
            hidden_state = self.lstm_pass(self.prev_disc_dist, actions[0, index].unsqueeze(dim=0).unsqueeze(dim=0)) # Shape: (1, 1, 2048)
            disc_dist_from_hidden_base, disc_dist_from_hidden  = self.dist_from_hidden(hidden_state) # Shape (1, 1, X)
            decoded: torch.Tensor = self.decoder_pass(disc_dist_from_hidden_base) # Shape: (1, observation_dim_0, observation_dim_1)
        ...
        return torch.cat(disc_dist_encoder, dim=1), torch.cat(disc_dist_hidden, dim=1), torch.cat(reconstructions, dim=1), torch.cat(rewards, dim=1), torch.cat(ends, dim=1)
    
    @njit
    def forward_train_actor_critic(self, observation: torch.Tensor):
        
        encoded: torch.Tensor = self.encoder_pass(observation)
        disc_dist, _: torch.Tensor = self.dist_from_encoder(encoded)
